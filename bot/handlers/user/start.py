...
@router.message(
    CommandStart(
        magic=F.args.regexp(
            r"^ref_((?:[uU][A-Za-z0-9]{9})|(?:[A-Za-z0-9]{9})|\d+)$"
        ).as_("ref_match")
    )
)
...
@router.message(
    CommandStart(
        magic=F.args.regexp(r"^(?!ref_|promo_)([A-Za-z0-9_\-]{2,64})$").as_(
            "ad_param_match"
        )
    )
)
...
async def verify_channel_subscription_callback(
    ...
    if not settings.DISABLE_WELCOME_MESSAGE:
        welcome_text = _(
            key="welcome", user_name=hd.quote(callback.from_user.full_name)
        )
        if callback.message:
            try:
    ...
    try:
        await callback.answer(
            _(key="channel_subscription_verified_success"), show_alert=True
        )
    except Exception as exc:
        logging.debug("Suppressed exception in bot/handlers/user/start.py: %s", exc)
...
async def about_command_handler(
    event: Union[types.Message, types.CallbackQuery],
    i18n_data: dict,
    settings: Settings,
):
    ...
    text_to_send = _(key="about_service")
    reply_markup = get_tos_and_pp_keyboard(i18n, current_lang, settings)
