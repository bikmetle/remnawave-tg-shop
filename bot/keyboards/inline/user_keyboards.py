...
def get_main_menu_inline_keyboard(
    ...
    if settings.TERMS_OF_SERVICE_URL or settings.PRIVACY_POLICY_URL:
        builder.row(
            InlineKeyboardButton(
...
def get_tos_and_pp_keyboard(
    i18n_instance, current_lang: str, settings: Settings
) -> InlineKeyboardMarkup:
    _ = lambda key, **kwargs: i18n_instance.gettext(current_lang, key, **kwargs)
    builder = InlineKeyboardBuilder()
    builder.button(
        text=_(key="terms_of_service_button"),
        url=settings.TERMS_OF_SERVICE_URL,
    )
    builder.button(
        text=_(key="privacy_policy_button"),
...
def get_back_to_main_menu_markup(
    ...
    if callback_data:
        builder.button(
            text=_(key="back_to_main_menu_button"), callback_data=callback_data
        )
    else:
        builder.button(
            text=_(key="back_to_main_menu_button"),
